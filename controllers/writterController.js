...
export const createWritter = async (req, res) => {
  let { age, city, province, country, qualifications, designation, purpose } =
    req.body.data;
  const { userId } = req.body;
  if (!age || !city || !country || !province || !qualifications || !purpose) {
  ...
  try {
    ...
    if (result) {
      ...
      res
        ...
        .json({ msg: "Writter Image Not Found !" });
    }
  } catch (error) {
    res
      .status(StatusCodes.INTERNAL_SERVER_ERROR)
...
export const getAllDisapprovedWritters = async (req, res) => {
  let DissapprovedWritters = await WritterModel.find({
    isApproved: false,
    isDisapproved: true,
  });
...
export const getAllRejectedRequests = async (req, res) => {
  let rejectedRequests = await WritterModel.find({
    isApproved: false,
    isRejected: true,
  });
...
export const getSingleWritter = async (req, res) => {
  try {
    ...
    if (!writer) {
      res
        .status(StatusCodes.NOT_FOUND)
        .json({ msg: "This is Invalid WritterID" });
    }
